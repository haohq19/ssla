...
from utils.ela_utils import check_shared_mem, input_guard
...
def chunk_local_cumsum_scalar_kernel(
    ...
    # i_t is the global chunk index, i_h is the head index
    i_t, i_h = tl.program_id(0), tl.program_id(1)
    # i_n is the sequence index, i_t is the chunk index within the sequence 
...
def chunk_local_cumsum_vector_kernel(
    ...
    # i_s is the width block index, i_t is the global chunk index, i_bh is the head index
    i_s, i_t, i_h = tl.program_id(0), tl.program_id(1), tl.program_id(2)
    # i_n is the sequence index, i_t is the chunk index within the sequence
    i_n, i_t = tl.load(chunk_indices + i_t * 2).to(tl.int32), tl.load(chunk_indices + i_t * 2 + 1).to(tl.int32)
    bos, eos = tl.load(cu_seqlens + i_n).to(tl.int32), tl.load(cu_seqlens + i_n + 1).to(tl.int32)
    T = eos - bos
    ...
    o_i = tl.arange(0, BT)
    if REVERSE:
        m_s = tl.where(o_i[:, None] <= o_i[None, :], 1., 0.)  # upper triangular matrix
    else:
        m_s = tl.where(o_i[:, None] >= o_i[None, :], 1., 0.)   # lower triangular matrix
...
def chunk_local_cumsum_vector(
    ...
    g_org, g = g, torch.empty_like(g, dtype=output_dtype or g.dtype)
    def grid(meta): return (triton.cdiv(meta['S'], meta['BS']), NT, H)
...
def chunk_local_cumsum(
    ...
    **kwargs
    ...
    # calculate the local cumsum for each chunk
    if len(g.shape) == 2:   # T, H
        return chunk_local_cumsum_scalar(g, chunk_size, cu_seqlens, reverse, output_dtype)
    elif len(g.shape) == 3: # T, H, C
        return chunk_local_cumsum_vector(g, chunk_size, cu_seqlens, reverse, output_dtype)
    else:
        raise ValueError(
            f"Unsupported input shape {g.shape}. "
            f"which should be (T, H, D) or (T, H)"
        )
